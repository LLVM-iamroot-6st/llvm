...
static int gettok() {
  ...
  //def func() -> tok_def tok_identifier(func:IndentifierStr)
...
Value *IfExprAST::Codegen() {
  ...
  //line:768 ThenBB = Builder.GetInsertBlock() 와 같은 경우 인듯??? 
...
Function *PrototypeAST::Codegen() {
 ...
 /*
    ...
    1 .NG Case
     def func()
       body;
     def func(); //llvm renames to another one by adding suffix. e.g) func1
     def func(); //llvm renames to another one by adding suffix. e.g) func2

    2. OK Case
      def func();
    
      def func() //llvm renames to another one by adding suffix.
        body;

    이 언어에서는 함수의 정의가 선언앞에 올수 없도록 하기때문에
    이에 대한 확인을 하는 코드이다.
    - Function::Create시 같은 이름의 함수가 있을 경우 llvm은 그 이름에 suffix 를 붙여 구분한다.
   
  */
  std::cout<<F->getName().str()<<std::endl; 
  if (F->getName() != Name) {
    // Delete the one we just made and get the existing one.
    std::cout<<"name conflicted!!"<<std::endl;
    F->eraseFromParent();
